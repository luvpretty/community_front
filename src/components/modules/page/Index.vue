...
<div class="layui-box layui-laypage layui-laypage-default">
 ...
 <template v-for="(item, index) in pages">
    <a @click="changeIndex(index)" v-if="item>=(current + 1 - 2) && item <=(current + 1 + 2)"
    href="javascript:;"
    :key="'page' + index"
 ...
 <a class="layui-laypage-next" v-show="showEnd"
 :class="{'layui-disabled': current === pages.length - 1}" @click.prevent="end()">
...
export default {
  ...
  methods: {
    ...
    home () {
      this.$emit('changCurrent', 0)
    },
    end () {
      this.$emit('changCurrent', this.pages.length - 1)
    },
    prev () {
      let cur = 0
      if (this.current - 1 < 0) {
